...
\Address{
  ...
  Department of Statistics\\
  Yale University\\
  Email: \email{taylor.arnold@yale.edu}
}
...
In a future paper we will reveal the details of various implementation strategies used in 
...
reasonably solved with the path algorithm. 
...
A common variant of the fused lasso is employs an additional $\ell_1$ penalty on
