...
fusedlasso <- function(y, X, D, graph, gamma=0, approx=FALSE,
  ...
  if (gamma==0) {
    if (is.null(X)) out = dualpathFused(y,D,approx,maxsteps,minlam,tol,verbose,fileback)
    else out = dualpathFusedX(y,X,D,approx,maxsteps,minlam,tol,verbose,fileback)
  }
  else {
    # Construct the proper D matrix
    D0 = D
    D = rBind(D0,gamma*diag(1,ncol(D0)))
    
    if (is.null(X)) out = dualpathFusedL1(y,D,D0,gamma,approx,maxsteps,minlam,tol,verbose,fileback)
    else out = dualpathFusedL1X(y,X,D,D0,gamma,approx,maxsteps,minlam,tol,verbose,fileback)
  }
